function createCharts() {
	...
	dispOrder = JSON.parse(dispOrder);
	paramNames = JSON.parse(paramNames);
...
function drawCharts(charts, options, data, units, paramNames) {
	for (let i in data) {
		...
		charts[i].innerHTML = `${paramName + " [" + units[i] + "]"}: ${
			data[i]
		}`;
		if (!options[i]) {
			charts[i].className = "black-text";
		} else if (
			data[i] > options[i]["rangeEnd"] ||
			data[i] < options[i]["rangeStart"]
		) {
